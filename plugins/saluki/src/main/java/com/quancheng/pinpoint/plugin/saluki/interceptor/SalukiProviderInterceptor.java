...
public class SalukiProviderInterceptor extends SpanSimpleAroundInterceptor {
    ...
    protected Trace createTrace(Object arg0, Object[] arg1) {
        ...
        String transactionId = RpcContext.getContext().getAttachment(SalukiConstants.META_TRANSACTION_ID);
        if (transactionId == null) {
            return traceContext.newTraceObject();
        }
        long parentSpanID = NumberUtils.parseLong(RpcContext.getContext().getAttachment(SalukiConstants.META_PARENT_SPAN_ID),
                                                  SpanId.NULL);
        long spanID = NumberUtils.parseLong(RpcContext.getContext().getAttachment(SalukiConstants.META_SPAN_ID),
                                            SpanId.NULL);
        short flags = NumberUtils.parseShort(RpcContext.getContext().getAttachment(SalukiConstants.META_FLAGS),
                                             (short) 0);
